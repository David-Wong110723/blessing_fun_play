...
from common_utils import download_weight
    ...
    def __init__(self, name='mobilenet', weight_path=None, device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu"), confidence_threshold=0.99,
        ...
        if name == 'mobilenet':
            ...
            if weight_path is None:
                weight_path = os.path.join(os.path.dirname(file_name), '../../../models/face_tools/Retinaface/mobilenet.pth')
            else:
                weight_path = os.path.join(weight_path, "Retinaface/mobilenet.pth")
            assert os.path.isfile(weight_path), f"Retinaface weight file {weight_path} is not exsit."
            """
            ...
            if weight_path is None:
                weight_path = os.path.join(os.path.dirname(file_name), '../../../models/face_tools/Retinaface/resnet50.pth')
            else:
                weight_path = os.path.join(weight_path, "Retinaface/resnet50.pth")
            assert os.path.isfile(weight_path), f"Retinaface weight file {weight_path} is not exsit."
            """
            if os.path.isfile(weight_path) == False:
                os.makedirs(os.path.split(weight_path)[0], exist_ok=True)
