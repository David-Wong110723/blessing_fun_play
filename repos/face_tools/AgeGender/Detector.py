...
class AgeGenderEstimator:
    ...
    def __init__(self, name='full', weight_path=None, device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu")):
        ...
        if weight_path is None:
            ...
            weight_path = os.path.join(os.path.dirname(file_name), '../../../models/face_tools/AgeGender/ShufflenetFull.pth')
            assert os.path.isfile(weight_path), f"AgeGender weight file {weight_path} is not exsit."
            """
            ...
            weight_path = os.path.join(weight_path, "AgeGender/ShufflenetFull.pth")
